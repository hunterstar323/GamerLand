...
function renderStatusActions(
  ...
  const actions =
    context === 'purchase'
      ? [
          { value: '1', label: 'Marcar entregado' },
          { value: '2', label: 'Cancelar' },
        ]
      : [{ value: '2', label: 'Cancelar venta' }];

  if (purchase.swEstado === '1' || purchase.swEstado === '2') {
    return '';
  }

  return `
    <form class="mt-4 flex flex-wrap items-center gap-2" data-form="purchase-status" data-purchase-id="${purchase.id}">
      <select class="rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2 text-sm text-slate-100" name="swEstado">
        ${actions
          .map((action) => `<option value="${action.value}">${action.label}</option>`)
          .join('')}
      </select>
      <button class="ghost-button" type="submit">Actualizar estado</button>
    </form>
  `;
}
...
function renderPurchaseCard(
  purchase: Purchase,
  context: 'purchase' | 'sale' = 'purchase',
): string {
    ...
    <article class="rounded-3xl border border-white/10 bg-white/5 p-4">
      <div class="mb-3 flex items-center justify-between gap-4">
        ...
        <div class="text-right">
          ...
          <strong class="mt-2 block text-lg text-amber-300">${currency.format(purchase.total)}</strong>
        </div>
      </div>
      <ul class="space-y-2">${items}</ul>
      ${renderStatusActions(purchase, context)}
    </article>
  `;
}
...
export function renderProfileView(state: AppState): string {
  ...
  const purchasesHtml = state.purchases.length
    ? state.purchases.map((purchase) => renderPurchaseCard(purchase, 'purchase')).join('')
    : '<p class="empty-state">Aún no has realizado compras.</p>';

  const salesHtml = state.sales.length
    ? state.sales.map((purchase) => renderPurchaseCard(purchase, 'sale')).join('')
    : '<p class="empty-state">Aún no tienes ventas registradas.</p>';
  ...
  return `
    <section class="panel p-6 lg:p-8 animate-rise">
      ...
      <div class="mt-8">
        <h3 class="mb-4 text-xl font-semibold text-white">Historial de ventas</h3>
        <div class="grid gap-3 lg:grid-cols-2">${salesHtml}</div>
      </div>
    </section>
  `;
}
