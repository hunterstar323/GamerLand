...
function render(): void {
  ...
  pendingRender = false;
  pendingHtml = renderApp(state);

  if (pendingHtml === lastRenderedHtml) {
    return;
  }
  ...
  renderFrame = window.requestAnimationFrame(() => {
    ...
    appRoot.innerHTML = pendingHtml;
    lastRenderedHtml = pendingHtml;
...
async function refreshPrivateData(): Promise<void> {
  ...
  const [user, myProducts, purchases, sales] = await Promise.all([
    api.me(session.token),
    api.getMyProducts(session.token),
    api.getMyPurchases(session.token),
    api.getMySales(session.token),
  ]);
  ...
  if (session.user.role === '1') {
    const [users, allPurchases] = await Promise.all([
      api.getUsers(session.token),
      api.getPurchases(session.token),
    ]);

    state.users = users;
    state.allPurchases = allPurchases;
  } else {
    state.users = [];
    state.allPurchases = [];
  }
...
async function withBusy(work: () => Promise<void>): Promise<void> {
  ...
  try {
    ...
    if (pendingRender || renderBatchDepth === 0) {
      render();
    }
  }
}
...
root.addEventListener('submit', async (event) => {
  ...
  if (formName === 'purchase-status') {
    await withBusy(async () => {
      try {
        const session = requireSession();
        const purchaseId = Number(form.dataset.purchaseId ?? '0');
        const swEstado = values.swEstado as PurchaseStatus;

        await api.updatePurchaseStatus(purchaseId, swEstado, session.token);
        await refreshPublicData();
        await refreshPrivateData();
        setNotice('Estado de compra actualizado.', 'success');
      } catch (error) {
        setNotice(getErrorMessage(error), 'error');
      }
    });
